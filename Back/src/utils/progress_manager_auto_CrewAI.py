...
from ..progress.debate_2_crewai import Debate_2_CrewAI
...
class ProgressManagerAutoCrewAI:
    def __init__(self, participant_factory:ParticipantFactory,
                        ...
                        generate_text_config: dict, api_key):
    ...
    def create_progress(self, progress_type:str, participant:dict) -> dict:
        """
        ...
        gemini_api = GeminiAPI(api_key=self.api_key)
        user_prompt = "Return a single debate topic in one sentence. Keep it concise and argumentative. No extra details."
        topic = gemini_api.generate_text(user_prompt,temperature=0.5,max_tokens=100)
        print(topic)
        ...
        if progress_type == "debate":
            ...
            debate = Debate_2_CrewAI(participant=generated_participant, generate_text_config=self.generate_text_config["debate"])
